...
parser.add_argument('--mode', '-mo', required=False,
                    default="CHK", type=str, help='mode: CHK | PP | EXP')
...
if args.mode == 'EXP':
    ...
    ensure_dir(postpondDir)  # 쓸일이 없넹 필요하면 넣자

    # list는 이미 불렀음
    # 전체 리스트 순회
    for file in tqdm(list_Excluded_byFig["filename"]):
        # type에서 IN이라고 되어 있는 file이 있으면
        if (list_Excluded_byFig.loc[list_Excluded_byFig.filename == file, "type"] == "IN").bool():
            # 순회하면서 "IN"인 것만 raw에 저장하기
            if not os.path.exists(os.path.join(includeDir, file)):  # 파일이 이미 있으면 그냥 지나감
                shutil.copyfile(os.path.join(rawDir, file),
                                os.path.join(includeDir, file))

        # type에서 IN이라고 되어 있는 file이 있으면
        elif (list_Excluded_byFig.loc[list_Excluded_byFig.filename == file, "type"] == "EXC").bool():
            # 순회하면서 "IN"인 것만 raw에 저장하기
            if not os.path.exists(os.path.join(includeDir, file)):  # 파일이 이미 있으면 그냥 지나감
                shutil.copyfile(os.path.join(rawDir, file),
                                os.path.join(excludeDir, file))
        # type에서 IN이라고 되어 있는 file이 있으면
        elif (list_Excluded_byFig.loc[list_Excluded_byFig.filename == file, "type"] == "PP").bool():
            # 순회하면서 "IN"인 것만 raw에 저장하기
            if not os.path.exists(os.path.join(includeDir, file)):  # 파일이 이미 있으면 그냥 지나감
                shutil.copyfile(os.path.join(rawDir, file),
                                os.path.join(postpondDir, file))
    print("Done!")
        ...
        if not (list_Excluded_byFig.loc[list_Excluded_byFig.filename == datum, "type"] == 'TBD').empty:
            ...
            while True:
                ...
                if keyboard.read_key(suppress=True) == "q":  # 포함시킬 파일
                    list_Excluded_byFig.loc[list_Excluded_byFig.filename ==
                                            datum, "type"] = "IN"
                    list_Excluded_byFig.loc[list_Excluded_byFig.filename ==
                                            datum, "checker"] = chk_name
