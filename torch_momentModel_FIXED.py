...
dataType = 'moBWHT'  # Type of data to predict: 'angle' or 'moBWHT'
...
for numFold in range(totalFold):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}")):
            ...
            # This is done because not all batches may have the same size
            train_loss += loss.item() * data.size(0)
            
            # Calculate and accumulate nRMSE for each axis (total loss sum for this batch)
            ...
            # Optional: Log training progress at specified intervals (currently commented out)
        ...
        writer_train.add_scalar('loss(MAE)
writer_train.add_hparams(
        ...
        # Set model to evaluation mode (disables dropout, switches batch norm to eval mode)
        my_model.eval()
        
        # Disable gradient computation for evaluation (saves memory and speeds up computation)
        with torch.no_grad():
            ...
            for data, target in test_loader:
                ...
                test_x_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'x', load_scaler4Y).item()  # Total loss sum for this batch
                test_y_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'y', load_scaler4Y).item()  # Total loss sum for this batch
                test_z_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'z', load_scaler4Y).item()  # Total loss sum for this batch
