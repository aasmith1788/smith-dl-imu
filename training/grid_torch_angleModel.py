...
######### 설정 영역 ########
exp_name = 'torch_test_env_imu' # 실험 이름
...
def nRMSE_Axis_TLPerbatch(pred, target,axis,load_scaler4Y):
    ...
    for bat in range(batchNum): # batch 내를 순회
        ...
        pred_axis = (pred_axis - torch.tensor(load_scaler4Y.min_, device=device)) / torch.tensor(load_scaler4Y.scale_,device=device)
        target_axis = (target_axis - torch.tensor(load_scaler4Y.min_,device=device)) / torch.tensor(load_scaler4Y.scale_,device=device)
