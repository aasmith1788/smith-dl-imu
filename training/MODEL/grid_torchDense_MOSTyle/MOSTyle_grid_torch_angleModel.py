...
######### 설정 영역 ########
exp_name = "MOSTyle_torch_20220706"  # 실험 이름 혹은 오늘 날짜
modelVersion = "MOSTyle_Dense_1st_torch"
nameDataset = "IWALQQ_AE_MOSTyle_2nd"
...
class Mlp(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Linear(1818, 6000)
...
def nRMSE_Axis_TLPerbatch(pred, target, axis, load_scaler4Y):
    ...
    for bat in range(batchNum):  # batch 내를 순회
        pred_axis = torch.transpose(
            torch.reshape(torch.squeeze(pred[bat]), [3, -1]), 0, 1
        )[:, axis]
        target_axis = torch.transpose(
            torch.reshape(torch.squeeze(target[bat]), [3, -1]), 0, 1
        )[:, axis]
        pred_axis = (
            pred_axis - torch.tensor(load_scaler4Y.min_, device=device)
        ) / torch.tensor(load_scaler4Y.scale_, device=device)
        target_axis = (
            target_axis - torch.tensor(load_scaler4Y.min_, device=device)
        ) / torch.tensor(load_scaler4Y.scale_, device=device)
        nRMSE = (
            100
            * torch.sqrt(torch.mean(torch.square(pred_axis - target_axis)))
            / (torch.max(target_axis) - torch.min(target_axis))
        )
...
for opt1 in range(0, len(list_learningRate)):
    for opt2 in range(0, len(list_batch_size)):
        ...
        for numFold in range(totalFold):
            ...
            x = torch.rand(1, 1818, device=device)
