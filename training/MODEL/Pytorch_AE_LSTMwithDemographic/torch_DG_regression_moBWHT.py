...
######### 설정 영역 ########
# 실험 관련 세팅
exp_name = 'DG_tor_denseRg_20220607_vaelstm_1layer'  # 실험 이름 혹은 오늘 날짜
        ...
        for opt3 in range(0,len(list_embedding_dim)):
            for opt4 in range(0,len(list_batch_size)):
                ...
                for numFold  in range(totalFold):
                    ...
                    loadmodelname = join(dir_savedVAE,f'{load_dataType}_{embedding_dim}_{numFold}_fold')
                    ...
                    data_test  = DatasetWithDG4regressor(dataSetDir, dataType, 'test', numFold)
                    ...
                    data_imu, data_dg, target = data_imu.to(device), data_dg.to(device), target.to(device)
                    ...
                    for epoch in range(epochs):
                        ...
                        train_y_nRMSE = 0
                        train_z_nRMSE = 0
                        for batch_idx, (data_imu, data_dg ,target) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs}")):
                            ...
                            loss.backward()
                            optimizer.step()
                            train_loss += loss.item() * data_imu.size(0) # 이것은 모든 배치의 크기가 일정하지 않을 수 있기 때문에 이렇게 수행함! train_loss는 total loss of batch가 됨
                            train_x_nRMSE += nRMSE_Axis_TLPerbatch(output,target, 'x', load_scaler4Y, device).item() # 해당 배치에서의 총 loss의 합
                            train_y_nRMSE += nRMSE_Axis_TLPerbatch(output,target, 'y', load_scaler4Y, device).item() # 해당 배치에서의 총 loss의 합
                            train_z_nRMSE += nRMSE_Axis_TLPerbatch(output,target, 'z', load_scaler4Y, device).item() # 해당 배치에서의 총 loss의 합
                        ...
                        with torch.no_grad():  # autograd engine, 즉 backpropagatin이나 gradient 계산 등을 꺼서 memory usage를 줄이고 속도를 높임
                            for data_imu, data_dg, target in test_loader:
                                ...
                                test_x_nRMSE += nRMSE_Axis_TLPerbatch(output,target, 'x', load_scaler4Y, device).item()# 해당 배치에서의 총 loss의 합
                                test_y_nRMSE += nRMSE_Axis_TLPerbatch(output,target, 'y', load_scaler4Y, device).item() # 해당 배치에서의 총 loss의 합
                                test_z_nRMSE += nRMSE_Axis_TLPerbatch(output,target, 'z', load_scaler4Y, device).item() # 해당 배치에서의 총 loss의 합             
