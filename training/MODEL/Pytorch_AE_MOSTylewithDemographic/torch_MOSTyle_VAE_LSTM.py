...
######### 설정 영역 ########
# 실험 관련 세팅
exp_name = "MOSTyle_tor_vaelstm_20220705_IWALQQ_AE_MOSTyle_2nd"  # 실험 이름 혹은 오늘 날짜
modelVersion = "vaelstm_1st_torch_MOSTyle"
...
list_embedding_dim = [
    ...
    70,
    80,
]  # {0:10, 0:20, 0:30, 1:40, 2:50, 3:60, 4:70}
        ...
        for opt3 in range(0, len(list_embedding_dim)):
            for opt4 in range(0, len(list_batch_size)):
                ...
                for numFold in range(totalFold):
                    ...
                    # 매 fold마다 새로운 모델
                    my_model = RecurrentVariationalAutoencoder(
                        seq_len, num_features, embedding_dim, device
                    )
                    my_model.to(device)

                    # loss function and optimizer define
                    optimizer = torch.optim.NAdam(
                        my_model.parameters(), lr=learningRate
                    )

                    angle_train = Dataset4autoencoder(
                        dataSetDir, dataType, "train", numFold
                    )
                    angle_test = Dataset4autoencoder(
                        dataSetDir, dataType, "test", numFold
                    )
                    train_loader = DataLoader(
                        angle_train, batch_size=batch_size, shuffle=True
                    )
                    test_loader = DataLoader(
                        angle_test, batch_size=batch_size, shuffle=True
                    )
