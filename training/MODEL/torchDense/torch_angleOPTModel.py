...
exp_name = 'date_Dense_1st_torch_optimized'  # Experiment name for logging
modelVersion = 'Dense_1st_torch_optimized'    # Version identifier for the model
...
# UPDATED: Single output directory for all results (new optimized version)
output_base_dir = r'R:\KumarLab3\PROJECTS\wesens\Data\Analysis\smith_dl\IMU Deep Learning\Training_results\Dense_1st_Torch_Angle_optimized_mln'
...
# Define the Enhanced Multi-Layer Perceptron (MLP) model
class Mlp(nn.Module):
    def __init__(self):
        ...
        self.bn1 = nn.BatchNorm1d(6000)  # Batch normalization after first layer
        
        # Second fully connected layer: 6000 -> 4000 neurons
        self.layer2 = nn.Linear(6000, 4000)
        self.bn2 = nn.BatchNorm1d(4000)  # Batch normalization after second layer
...
for numFold in range(totalFold):
    ...
    with torch.no_grad():
        ...
        for data, target in train_loader:
            ...
            final_train_loss += loss.item() * data.size(0)
            final_train_x_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'x', load_scaler4Y).item()
            final_train_y_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'y', load_scaler4Y).item()
            final_train_z_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'z', load_scaler4Y).item()
        
        final_train_loss /= len(train_loader.sampler)
        final_train_x_nRMSE /= len(train_loader.sampler)
        final_train_y_nRMSE /= len(train_loader.sampler)
        final_train_z_nRMSE /= len(train_loader.sampler)
        ...
        for data, target in test_loader:
            ...
            final_test_x_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'x', load_scaler4Y).item()
            final_test_y_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'y', load_scaler4Y).item()
            final_test_z_nRMSE += nRMSE_Axis_TLPerbatch(output, target, 'z', load_scaler4Y).item()
        
        final_test_loss /= len(test_loader.sampler)
        final_test_x_nRMSE /= len(test_loader.sampler)
        final_test_y_nRMSE /= len(test_loader.sampler)
        final_test_z_nRMSE /= len(test_loader.sampler)
