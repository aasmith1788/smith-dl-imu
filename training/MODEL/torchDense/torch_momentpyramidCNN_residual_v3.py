...
RESULTS_DIR = join(BASE_RESULTS_DIR, 'momentpyramidCNN_RESIDUAL_v3')
...
# UPDATED HYPERPARAMETERS FOR v3 - WITH INITIAL EXPANSION
BEST_PARAMS = {
...
def multi_seed_ensemble_training():
    ...
    print(f"V3 changes: Initial expansion (42 -> {INITIAL_EXPANSION_CHANNELS}), then 4 layers with channels {BEST_PARAMS['channels']}")
    ...
    for fold in range(N_FOLDS):
        print(f"\n{'='*60}")
        print(f"TRAINING FOLD {fold} - RESIDUAL v3 ENSEMBLE (initial expansion + 4 layers)")
        print(f"{'='*60}")
    ...
    print("\n" + "="*80)
    print("RESIDUAL v3 ENSEMBLE TRAINING COMPLETED - SUMMARY ACROSS ALL FOLDS")
    print("="*80)
    ...
    print(f"\n{'='*80}")
    print("RESIDUAL v3 ENSEMBLE TRAINING WITH INITIAL EXPANSION COMPLETED!")
    print(f"{'='*80}")
    ...
    print("2. More feature maps than first ResidualBlock (64 > 24) for better initial representation")
    print("3. Maintains all v2 benefits (4 layers, 192 channels, residual connections)")
    print("4. Same optimal hyperparameters and noise augmentation")
    print("\nNext steps:")
    print("- Compare v3 (initial expansion) vs v2 (direct processing) performance")
    print("- Analyze if initial expansion improves feature learning")
