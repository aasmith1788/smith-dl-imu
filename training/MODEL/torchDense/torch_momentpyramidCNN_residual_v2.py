...
# RESIDUAL VERSION v2: Adds 4th layer with 192 channels for deeper hierarchical feature learning
...
# GAUSSIAN NOISE PARAMETERS - UPDATED WITH YOUR OPTIMAL VALUE
GAUSSIAN_NOISE_ENABLED = True  # Toggle noise augmentation
NOISE_STD_FRACTION = 0.100  # Your optimal noise fraction
...
# Output directories for ensemble training - V2 FOLDER
BASE_RESULTS_DIR = r'R:\KumarLab3\PROJECTS\wesens\Data\Analysis\smith_dl\IMU Deep Learning\Training_results'
RESULTS_DIR = join(BASE_RESULTS_DIR, 'momentpyramidCNN_RESIDUAL_v2')
MODELS_DIR = join(RESULTS_DIR, 'models')
LOGS_DIR = join(RESULTS_DIR, 'logs')
ENSEMBLE_PREDS_DIR = join(RESULTS_DIR, 'ensemble_predictions')

# UPDATED HYPERPARAMETERS FOR v2 - DEEPER NETWORK
BEST_PARAMS = {
    "kernels": [3, 7],  # Converted from '3-7' string
    "channels": [24, 48, 96, 192],  # V2: Added 4th layer with 192 channels
    "n_layers": 4,  # V2: Increased from 3 to 4
    ...
    "num_heads": 4,
    "noise_frac": 0.100  # Your optimal noise fraction
}
...
class PyramidAttnCNN(nn.Module):
    """Multi-scale CNN with shallow temporal pyramid, attention, and residual connections."""
...
if __name__ == '__main__' and '--skip_param_check' not in sys.argv:
    ...
    print(f"Model parameters (v2 - 4 layers, 192 channels): {param_count:,}")
...
def multi_seed_ensemble_training():
    """Multi-seed ensemble training using optimal hyperparameters with Gaussian noise and residual connections - V2."""
    ...
    print("="*80)
    print("RESIDUAL VERSION v2: DEEPER NETWORK (4 layers, 192 channels)")
    print("="*80)
    ...
    if GAUSSIAN_NOISE_ENABLED:
        print(f"  Noise STD fraction: {NOISE_STD_FRACTION:.3f} (your optimal value)")
    print(f"ARCHITECTURE: PyramidAttnCNN with RESIDUAL CONNECTIONS + 4th layer (192 channels)")
    print(f"V2 changes: n_layers=4, channels=[24, 48, 96, 192] for deeper hierarchical learning")
    print(f"Your optimal hyperparameters: {BEST_PARAMS}")
    print("="*80)
    ...
    config = {
        'timestamp': timestamp,
        'version': 'residual_v2',
        'architecture_change': 'Added 4th layer with 192 channels for deeper hierarchical feature learning',
        'step': 'multi_seed_ensemble_optimal_parameters_residual_v2',
        'n_seeds': N_SEEDS,
        ...
        'gaussian_noise': {
            ...
            'description': 'Your optimal Gaussian noise (0.100) added to inputs during training only'
        },
        'best_params': BEST_PARAMS,
        'data_split': 'full_train_no_validation',
        'note': 'Residual v2: Increased depth to 4 layers with 192 channels for better hierarchical feature learning'
    }
    ...
    for fold in range(N_FOLDS):
        print(f"\n{'='*60}")
        print(f"TRAINING FOLD {fold} - RESIDUAL v2 ENSEMBLE (4 layers, 192 channels)")
        print(f"{'='*60}")
        ...
        print(f"Full training set: {len(full_train_dataset)} samples (with optimal noise augmentation)")
        ...
        fold_results = {
            ...
            'seeds': ENSEMBLE_SEEDS,
            'architecture': 'PyramidAttnCNN_with_ResidualBlocks_v2',
            'gaussian_noise': {
                'enabled': GAUSSIAN_NOISE_ENABLED,
    ...
    print("\n" + "="*80)
    print("RESIDUAL v2 ENSEMBLE TRAINING COMPLETED - SUMMARY ACROSS ALL FOLDS")
    print("="*80)
    ...
    summary = {
        ...
        'v2_changes': {
            ...
            'benefit': 'Deeper hierarchical feature learning with increased capacity'
        },
        'average_metrics': {
            'train': {
    ...
    print(f"\n{'='*80}")
    print("RESIDUAL v2 ENSEMBLE TRAINING WITH GAUSSIAN NOISE COMPLETED!")
    print(f"{'='*80}")
    print(f"Results saved to: {RESULTS_DIR}")
    print(f"Models saved to: {MODELS_DIR}")
    print(f"Logs saved to: {LOGS_DIR}")
    print(f"Predictions saved to: {ENSEMBLE_PREDS_DIR}")
    print("\nV2 configuration used:")
    print(f"- Model: PyramidAttnCNN with RESIDUAL BLOCKS (skip connections)")
    print(f"- Architecture enhancement: 4 layers with 192 max channels (deeper hierarchical learning)")
    print(f"- Attention heads: {BEST_PARAMS['num_heads']}")
    print(f"- Kernels: {BEST_PARAMS['kernels']} (multi-scale convolution)")
    print(f"- Channels: {BEST_PARAMS['channels']} (progressive feature expansion to 192)")
    ...
    print(f"- Noise augmentation: {BEST_PARAMS['noise_frac']:.3f} (your optimal value)")
    print(f"- Training epochs: {ENSEMBLE_EPOCHS} (conservative for generalization)")
    print("\nKey benefits of v2:")
    print("1. Deeper network (4 layers vs 3) for more complex feature hierarchies")
    print("2. Higher channel capacity (192 vs 96) for richer representations")
    print("3. Residual connections enable effective training of deeper architecture")
    print("4. Same optimal hyperparameters and noise augmentation")
    print("\nNext steps:")
    print("- Compare v2 (deeper) vs v1 (baseline residual) performance")
    print("- Analyze if increased depth improves peak moment prediction")
    print("- Check for overfitting with train-test generalization gaps")
