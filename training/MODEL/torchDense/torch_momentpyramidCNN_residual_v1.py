...
# RESIDUAL VERSION v1: Adds residual connections (skip connections) to improve gradient flow
...
# Output directories for ensemble training - CHANGED TO SEPARATE FOLDER
BASE_RESULTS_DIR = r'R:\KumarLab3\PROJECTS\wesens\Data\Analysis\smith_dl\IMU Deep Learning\Training_results'
RESULTS_DIR = join(BASE_RESULTS_DIR, 'momentpyramidCNN_RESIDUAL_v1')
...
BEST_PARAMS = {
    ...
    "channels": [24, 48, 96],  # Converted from '24-48-96' string
    "n_layers": 3,
...
if __name__ == '__main__' and '--skip_param_check' not in sys.argv:
    ...
    print(f"Model parameters (with residual connections): {param_count:,}")
...
def multi_seed_ensemble_training():
    """Multi-seed ensemble training using optimal hyperparameters with Gaussian noise and residual connections."""
    ...
    print("="*80)
    print("RESIDUAL VERSION v1: MULTI-SEED ENSEMBLE TRAINING WITH SKIP CONNECTIONS")
    print("="*80)
    ...
    print(f"ARCHITECTURE: PyramidAttnCNN with RESIDUAL CONNECTIONS (skip connections)")
    ...
    config = {
        'timestamp': timestamp,
        'version': 'residual_v1',
        'architecture_change': 'Added residual blocks with skip connections',
        'step': 'multi_seed_ensemble_optimal_parameters_residual',
        'n_seeds': N_SEEDS,
        ...
        'note': 'Residual version: Added ResidualBlock class with skip connections to improve gradient flow and training stability'
    }
    ...
    for fold in range(N_FOLDS):
        print(f"\n{'='*60}")
        print(f"TRAINING FOLD {fold} - RESIDUAL ENSEMBLE WITH YOUR OPTIMAL PARAMETERS")
        print(f"{'='*60}")
        ...
        fold_results = {
            ...
            'seeds': ENSEMBLE_SEEDS,
            'architecture': 'PyramidAttnCNN_with_ResidualBlocks',
            'gaussian_noise': {
                'enabled': GAUSSIAN_NOISE_ENABLED,
    ...
    print("\n" + "="*80)
    print("RESIDUAL ENSEMBLE TRAINING COMPLETED - SUMMARY ACROSS ALL FOLDS")
    print("="*80)
    ...
    print(f"\n{'='*80}")
    print("RESIDUAL ENSEMBLE TRAINING WITH GAUSSIAN NOISE COMPLETED!")
    print(f"{'='*80}")
    print(f"Results saved to: {RESULTS_DIR}")
    print(f"Models saved to: {MODELS_DIR}")
    print(f"Logs saved to: {LOGS_DIR}")
    print(f"Predictions saved to: {ENSEMBLE_PREDS_DIR}")
    print("\nYour optimal configuration used:")
    print(f"- Model: PyramidAttnCNN with RESIDUAL BLOCKS (skip connections)")
    print(f"- Architecture enhancement: ResidualBlock for improved gradient flow")
    ...
    print("1. Your optimal hyperparameters from systematic search")
    print("2. Residual connections (skip connections) for better gradient flow")
    print("3. Multi-seed ensemble for robust predictions and uncertainty quantification")
    print("4. Your optimal Gaussian noise augmentation (0.100) for regularization")
    print("5. Conservative epoch count to avoid overfitting")
    print("6. TensorBoard logging for training analysis")
    print("\nNext steps:")
    print("- Compare results with baseline (non-residual) model")
    print("- Analyze if residual connections improve peak moment prediction")
    print("- Check TensorBoard logs for training stability improvements")
    print("- Evaluate if gradient flow is improved (monitor gradient norms)")
