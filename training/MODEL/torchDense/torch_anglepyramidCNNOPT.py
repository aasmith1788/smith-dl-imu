...
# GAUSSIAN NOISE PARAMETERS - UPDATED WITH OPTIMAL VALUE
GAUSSIAN_NOISE_ENABLED = True  # Toggle noise augmentation
NOISE_STD_FRACTION = 0.075  # Optimal noise fraction from your analysis (best generalization)
...
RESULTS_DIR = join(BASE_RESULTS_DIR, 'anglepyramidCNNOPT')
MODELS_DIR = join(RESULTS_DIR, 'models')
LOGS_DIR = join(RESULTS_DIR, 'logs')
ENSEMBLE_PREDS_DIR = join(RESULTS_DIR, 'ensemble_predictions')

# UPDATED OPTIMAL HYPERPARAMETERS FROM YOUR ANALYSIS
BEST_PARAMS = {
    "kernels": [3, 5],  # Converted from '3-5' string
    "channels": [32, 64, 128],  # Converted from '32-64-128' string
    "n_layers": 3,
    "pooling": "adaptive_max",
    "dropout_conv": 0.027234783291926995,
    "dropout_fc": 0.004429697351898755,
    "lr": 0.005733339427460197,
    "optimizer": "adamw",
    "loss": "huber",
    "batch_size": 16,
    "weight_decay": 0.00023930201903077885,
    "scheduler": "none",
    "grad_clip": 2.6509357919146987,
    "num_heads": 2,
    "noise_frac": 0.075  # Optimal noise fraction for best generalization
}
...
def multi_seed_ensemble_training():
    ...
    print("="*80)
    print("STEP 3: MULTI-SEED ENSEMBLE TRAINING WITH OPTIMAL GAUSSIAN NOISE AUGMENTATION")
    print("="*80)
    ...
    if GAUSSIAN_NOISE_ENABLED:
        print(f"  Noise STD fraction: {NOISE_STD_FRACTION:.3f} (optimal from your analysis)")
        print(f"  ✅ Best generalization gap: -0.0001±0.0007")
        print(f"  ✅ Best validation loss: 0.0108±0.0009")
    print(f"Updated optimal hyperparameters: {BEST_PARAMS}")
    print("="*80)
    ...
    config = {
        'timestamp': timestamp,
        'step': 'multi_seed_ensemble_optimal_gaussian_noise',
        'n_seeds': N_SEEDS,
        ...
        'gaussian_noise': {
            ...
            'description': 'Optimal Gaussian noise from your analysis (0.075) added to inputs during training only',
            'performance': {
                'validation_loss': '0.0108±0.0009',
                'generalization_gap': '-0.0001±0.0007',
                'recommendation': 'Best for both generalization and validation loss'
            }
        },
        'best_params': BEST_PARAMS,
        'data_split': 'full_train_no_validation',
        'note': 'Step 3 of refinement ladder: multi-seed ensemble with your optimal hyperparameters and Gaussian noise augmentation'
    }
    ...
    for fold in range(N_FOLDS):
        print(f"\n{'='*60}")
        print(f"TRAINING FOLD {fold} - ENSEMBLE WITH YOUR OPTIMAL PARAMETERS")
        print(f"{'='*60}")
        ...
        fold_results = {
            ...
            'gaussian_noise': {
                ...
                'optimal': True,
                'analysis_result': 'Best generalization and validation loss at 0.075'
            },
    ...
    print("\n" + "="*80)
    print("OPTIMAL ENSEMBLE TRAINING WITH GAUSSIAN NOISE COMPLETED - SUMMARY ACROSS ALL FOLDS")
    print("="*80)
    ...
    summary = {
        ...
        'gaussian_noise': {
            ...
            'std_fraction': BEST_PARAMS['noise_frac'],
            'optimal': True,
            'analysis_result': 'Best generalization gap (-0.0001±0.0007) and validation loss (0.0108±0.0009) at 0.075',
    ...
    print(f"\n{'='*80}")
    print("OPTIMAL ENSEMBLE TRAINING WITH GAUSSIAN NOISE COMPLETED!")
    print(f"{'='*80}")
    ...
    print(f"- Model: PyramidAttnCNN with {BEST_PARAMS['num_heads']} attention heads")
    print(f"- Kernels: {BEST_PARAMS['kernels']} (multi-scale convolution)")
    print(f"- Channels: {BEST_PARAMS['channels']} (progressive feature expansion)")
    ...
    print(f"- Noise augmentation: {BEST_PARAMS['noise_frac']:.3f} (optimal from your analysis)")
    print(f"- Training epochs: {ENSEMBLE_EPOCHS} (conservative for generalization)")
    print("\nKey benefits achieved:")
    print("1. Your optimal hyperparameters from systematic analysis")
    print("2. Multi-seed ensemble for robust predictions and uncertainty quantification")
    print("3. Optimal Gaussian noise augmentation (0.075) for best generalization")
    print("4. Conservative epoch count to avoid overfitting")
    print("5. TensorBoard logging for training analysis")
    print("\nYour noise analysis results:")
    print(f"✅ Noise level 0.075: Best generalization gap (-0.0001±0.0007)")
    print(f"✅ Noise level 0.075: Best validation loss (0.0108±0.0009)")
    print(f"✅ Outperformed no noise (0.000) and higher noise levels")
    print("\nNext steps:")
    print("- Analyze TensorBoard logs to confirm 15 epochs is appropriate")
    print("- Compare ensemble predictions for uncertainty quantification")
    print("- Evaluate on external test sets if available")
    print("- Consider post-processing techniques for further improvement")
