...
RESULTS_DIR = r'R:\KumarLab3\PROJECTS\wesens\Data\Analysis\smith_dl\IMU Deep Learning\Training_results\momentpyramidCNNSEARCHOV'
...
def retrain_folds(best_params):
    ...
    for fold in range(N_FOLDS):
        ...
        # Convert kernel and channel parameters if provided as strings
        kernels = (
            tuple(int(x) for x in best_params["kernels"].split("-"))
            if isinstance(best_params["kernels"], str)
            else best_params["kernels"]
