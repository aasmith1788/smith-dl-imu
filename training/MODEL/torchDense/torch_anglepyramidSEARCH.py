...
RESULTS_DIR = r'R:\KumarLab3\PROJECTS\wesens\Data\Analysis\smith_dl\IMU Deep Learning\Training_results\anglepyramidCNN'
...
def sample_params(trial):
    channels = trial.suggest_categorical(
        "channels", ["24-48-96", "32-64-128", "48-96-192"]
    )
    kernels = trial.suggest_categorical("kernels", ["3-7", "3-5-7"])
    ...
    p = dict(
        ...
        # regularisation
        dropout_conv=trial.suggest_float("dropout_conv", 0.00, 0.15),
        dropout_fc=trial.suggest_float("dropout_fc", 0.00, 0.02),

        # optimisation
        lr=trial.suggest_float(
            "lr", BASE_PARAMS["lr"] / 3, BASE_PARAMS["lr"] * 3, log=True
        ),
        weight_decay=trial.suggest_float("weight_decay", 1e-5, 5e-4, log=True),

        # augmentation
        noise_frac=trial.suggest_float("noise_frac", 0.05, 0.10),
    )
    p.update(dict(
        n_layers=3,
        pooling="adaptive_max",
        ...
        optimizer="adamw",
        loss="huber",
        grad_clip=BASE_PARAMS["grad_clip"],
    ))
    return p
