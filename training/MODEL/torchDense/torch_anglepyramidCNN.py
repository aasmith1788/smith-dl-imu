...
if __name__ == '__main__' and '--skip_param_check' not in sys.argv:
    tmp_model = PyramidAttnCNN(**{k: BEST_PARAMS[k] for k in ['kernels', 'channels', 'n_layers', 'dropout_conv', 'dropout_fc']})
    assert count_parameters(tmp_model) < MAX_PARAMETERS, \
        f"Model too large: {count_parameters(tmp_model):,} parameters"
